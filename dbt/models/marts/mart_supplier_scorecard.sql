...
components as (
    select * from {{ source('firepoint', 'components') }}
)

select s.supplier_id, s.supplier_name, s.country,
    count(po.order_id) as total_orders, sum(po.quantity) as total_units_ordered,
    avg(po.lead_time_days) as avg_lead_time_days, sum(case when po.status = 'delivered' then 1 else 0 end) as delivered_count,

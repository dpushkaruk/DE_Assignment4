...
select e.department_id, date_trunc('month', a.attendance_date) as attendance_month,
    ...
    lag(round(sum(case when a.status = 'present' then 1 else 0 end) * 100.0 / count(*), 2)) over (
        partition by e.department_id order by date_trunc('month', a.attendance_date)) as prev_month_attendance_rate
from attendance a
left join employees e on a.employee_id = e.employee_id
group by e.department_id, date_trunc('month', a.attendance_date)